...
select FirstName from Employees WHERE Country='India' and LastName LIKE 'K%';
--2. Find the names of Rooms with a capacity over 50.
select RoomName from Rooms WHERE Capacity>50;
--3. Find the IDs of meeting with a duration more than 3 hours.
select MeetingID FROM Meetings WHERE DATEDIFF(HOUR,TimeFrom,TimeTo) > 3
--4. Find the IDs of rooms with a capacity over 50.
select RoomID FROM Rooms WHERE Capacity>50;
--5. Display the highest and the lowest capacities among rooms.
SELECT MAX(Capacity) AS 'highest capacities' ,MIN(Capacity) AS 'lowest capacities' FROM Rooms;
--6. Find the number of employees from India.
SELECT COUNT(EmployeeID) AS 'number of employees' FROM Employees WHERE Country='India';
--7. Find the number of employees from different countries.
select Country,count(Country) from Employees GROUP BY Country;
--8. Find the country which has more than 20 employees from.
SELECT Country,COUNT(country) FROM Employees GROUP BY Country HAVING COUNT(Country)>20;
--9. Find the number of rooms used for each meeting title.
SELECT MeetingTitle,COUNT(*) FROM Meetings GROUP BY MeetingTitle
--10. Find all meeting titles that have been held in more than one room, along with thenumber of rooms for each title.
SELECT MeetingTitle, COUNT(DISTINCT RoomID) AS room_count FROM meetings GROUP BY MeetingTitle HAVING COUNT(DISTINCT RoomID) > 1;
